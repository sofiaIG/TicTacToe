...
def adding_decision_into_board(value, board, character):
    """
    Add a player's decision to the baord and return the updated board
    """
...
while True:
    ...
    if win_game(the_board, X) is True or winning_diagonally(the_board, X) is True:
        display_board(the_board)
        break
    flat = [i for sublist in the_board for i in sublist]  # This is not working
    if set(flat) == {X, O}:
        print("The game is finished")
        break
    opponents_move(the_board)
    if win_game(the_board, O) is True or winning_diagonally(the_board, O) is True:
        display_board(the_board)
        break
